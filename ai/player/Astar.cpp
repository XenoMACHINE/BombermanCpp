...
Astar::Astar(const int n, const int m, const int xplayer, const int yplayer) : n(n), m(m), xPlayer(xplayer), yPlayer(yplayer)
{
    this->map = vector<vector<int>> (m, vector<int>(n, 0));
    this->closed_nodes_map = vector<vector<int>> (m, vector<int>(n, 0));
    this->open_nodes_map = vector<vector<int>> (m, vector<int>(n, 0));

    this->dir_map = vector<vector<int>> (m, vector<int>(n, 0));
}
...
// A-star algorithm.
string Astar::pathFindTmp(const int &xStart, const int &yStart, const int &xFinish, const int &yFinish)
{
...
string Astar::findPath(const int &xEnd, const int &yEnd)
{
    return pathFindTmp(this->xPlayer, this->yPlayer, xEnd, yEnd);
}